filterdimtabs<- function(fact) {
	currdims<- subset(M$tjoins, `Fact Table`==fact, select=`Dimension Table`) 
	currdims<- currdims[,1]
	setdiff(M$dm$dimtabs, currdims)
	}

addjoin<- function(fact, dim, fcol, dcol) {
...
dmodel<- function(input, output, session) {
	ns<- session$ns
	...
	observeEvent(input$factab, ignoreInit=T, ignoreNULL=T, {
		req(input$factab)
		dims<- filterdimtabs(input$factab)
